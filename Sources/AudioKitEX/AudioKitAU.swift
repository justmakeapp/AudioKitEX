...
open class AudioKitAU: AUAudioUnit {
    ...
    private lazy var auInputBusArray: AUAudioUnitBusArray = .init(
        audioUnit: self,
        busType: .input,
        busses: inputBusArray
    )
    ...
    private lazy var auOutputBusArray: AUAudioUnitBusArray = .init(
        audioUnit: self,
        busType: .output,
        busses: outputBusArray
    )
    ...
    override public init(componentDescription: AudioComponentDescription,
        ...
        for _ in 0 ..< inputBusCountDSP(dsp) {
            try inputBusArray.append(AUAudioUnitBus(format: format))
        }

        // All AudioKit nodes have one output bus.
        try outputBusArray.append(AUAudioUnitBus(format: format))
